 ...
 *
 */
public class OneToManyResultTransformer extends AliasedTupleSubsetResultTransformer {
	...
	public OneToManyResultTransformer(Class<?> resultClass) {
	...
	public Object transformTuple(Object[] tuple, String[] aliases) {
		...
		for (Class<?> subclass : subclassToAlias.keySet()) {
			Class<?> transformType=subclass;
			if(subclass.equals(List.class))
				transformType=ImItem.class;
			ResultTransformer subclassTransformer = new AliasToBeanResultTransformer(transformType);
			...
			try{
				...
				if(subclass.equals(List.class))
				{
					List<ImItem> lsItems=new ArrayList<ImItem>();
					lsItems.add((ImItem)subObject);
					accessor.getSetter(resultClass, (String)subclassToAlias.get(subclass).get(2)).set(root,lsItems, null);
				}
				else{
